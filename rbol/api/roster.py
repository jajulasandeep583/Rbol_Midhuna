# Vendored from HRMS v16.7.1 hrms/api/roster.py so the RBOL roster keeps
# working regardless of the HRMS version installed. The small HRMS helper
# functions it used (create_shift_assignment, get_or_insert_shift_schedule)
# are inlined below — this module must never import from hrms.
...
@frappe.whitelist()
def break_shift(assignment: str | Document, date: str) -> None:
...
def get_employee_holiday_list(employee: str, month_end: str) -> str | None:
	# older erpnext versions don't accept as_on
	try:
		return get_holiday_list_for_employee(employee, raise_exception=False, as_on=month_end)
	except TypeError:
...
def get_holidays(month_start: str, month_end: str, employee_filters: dict[str, str]) -> dict[str, list[dict]]:
	...
	for employee in frappe.get_list("Employee", filters=employee_filters, pluck="name"):
		if not (holiday_list := get_employee_holiday_list(employee, month_end)):
			continue
