// Tally-style Stock Summary — Warehouse > Item Group > Item drill-down
frappe.query_reports["Stock Summary"] = {
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (column.fieldname === "label") {
			if (data.node_type === "warehouse") {
				...
				var url = "/app/query-report/Stock Ledger"
		...
		if (column.fieldname === "close_val") {
			...
			value = "<span style='color:" + col + ";font-weight:" + (data.node_type !== "item" ? "700" : "500") + "'>" + display + "</span>";
		}
		return value;
	},
	...
	onload: function (report) {
		report.page.add_inner_button(__("Export to Excel"), function () {
			...
			a.href = url;
			a.download = "Stock_Summary_" + as_on + ".xls";
